<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://kit.fontawesome.com/24344f9aeb.js" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="index.css">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <title>Page d'acceuil</title>
</head>
<body>
    <header>
        <nav>  
            <ul>
                <li><button><a href="#etd">Etudiants</a></button></li>
                <li><button><a href="#adm">Administrateurs</a></button></li>
                <li><button><a href="#ftr">Contact de l'universite</a></button></li>
            </ul>
        </nav>

        <div class="par">
        <h1>Bienvenue sur le portail universitaire</h1>
        <h5>LE LIEN ENTRE ETUDIANTS ET ADMINISTRATEURS</h5>
        </div>
    </header>

    <main>
        <section class="portails" >
            <div class="portail etudiant" id="etd">
                <img src="etd.jpg" alt="Portail Étudiant">
                <a href="cnxet.html">Portail Étudiant</a>
            </div>

            <div class="portail admin" id="adm">
                <img src="adm.jpg" alt="Portail Administrateur">
                <a href="CnxA.html">Portail Administrateur</a>
            </div>
        </section>
    </main>
    
    <footer id="ftr">
        <p>© 2024 Université - Tous droits réservés</p>
        <p>Contactez-nous : <a href="mailto:contact@universite.com">contact@universite.com</a></p>
    </footer>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
</body>
</html>
